...
function Index() {
  ...
  const [reviewId, setReviewId] = useState([] as any);
  ...
  const [editReviewStars, setEditReviewStars] = useState(reviewId[0]?.stars);
  const [editReviewType, setEditReviewType] = useState(reviewId[0]?.type);
  const [editReviewProductId, setEditReviewProductId] = useState(
    reviewId[0]?.product?.id
  );
  const [editReviewUserId, setEditReviewUserId] = useState(
    reviewId[0]?.user?.id
  );
  ...
  const [filterStars, setFilterStars] = useState('reviewasc');
  const [filterType, setFilterType] = useState('reviewasc');
  const [filterProductId, setFilterProductId] = useState('productasc');
  const [filterUserId, setFilterUserId] = useState('userasc');
  const [search, setSearch] = useState('');

  useEffect(() => {
    try {
      axios.get(`https://quocson.fatcatweb.top/v2/review?page=${page}`).then((res) => {
  ...
  useEffect(() => {
    try {
      axios.get(`https://quocson.fatcatweb.top/v2/searchreview?search=searchall&sortBy=${search}`)
      .then((res: any) => {
        setReviews(res.data);
      })
    } catch (error) {
  ...
  useEffect(() => {
    ...
    try {
        axios.get('https://quocson.fatcatweb.top/users/search')
        .then((res:any) => {
            setUsers(res.data)
        })
    } catch (error) {
  ...
  const handleClickStars = () => {
    try {
      axios
        .get(
          `https://quocson.fatcatweb.top/v2/searchreview?search=${filterStars}&sortBy=stars`
        )
        .then((res: any) => {
          setReviews(res.data);
          if (filterStars === "reviewasc") {
            setFilterStars("reviewdesc");
          } else {
            setFilterStars("reviewasc");
          }
        });
    } catch (error) {
      console.log(error);
  ...
  const handleClickType = () => {
    try {
      axios
        .get(
          `https://quocson.fatcatweb.top/v2/searchreview?search=${filterType}&sortBy=type`
        )
        .then((res: any) => {
          setReviews(res.data);
          if (filterType === "reviewasc") {
            setFilterType("reviewdesc");
          } else {
            setFilterType("reviewasc");
          }
        });
    } catch (error) {
      console.log(error);
  ...
  const handleClickProductId = () => {
    try {
      axios
      .get(
        `https://quocson.fatcatweb.top/v2/searchreview?search=${filterProductId}&sortBy=id`
      )
      .then((res: any) => {
        setReviews(res.data);
        if (filterProductId === "productasc") {
          setFilterProductId("productdesc");
        } else {
          setFilterProductId("productasc");
        }
      });
    } catch (error) {
      console.log(error);
  ...
  const handleClickUserId= () => {
    try {
      axios
      .get(
        `https://quocson.fatcatweb.top/v2/searchreview?search=${filterUserId}&sortBy=id`
      )
      .then((res: any) => {
        setReviews(res.data);
        if (filterUserId === "userasc") {
          setFilterUserId("userdesc");
        } else {
          setFilterUserId("userasc");
        }
      });
    } catch (error) {
      console.log(error);
          ...
          <div className="flex">
            <div className="relative w-full ">
              <input
                ...
                placeholder="by id, stars , type, productId, userId..."
                required
              />
              <button
                type="submit"
      ...
      <div className="mx-auto">
        <Table hoverable={true}>
          <Table.Head>
            ...
            <Table.HeadCell>
            <a className="cursor-pointer" onClick={handleClickStars}>
                <div className="flex gap-1 items-center justify-end">
                  {filterStars === "reviewasc" ? (
                    <FaSortAmountDownAlt />
                  ) : (
                    <FaSortAmountUp />
                  )}
            ...
            <a className="cursor-pointer" onClick={handleClickType}>
                <div className="flex gap-1 items-center justify-end">
                  {filterType === "reviewasc" ? (
                    <FaSortAmountDownAlt />
                  ) : (
                    <FaSortAmountUp />
                  )}
            ...
            <Table.HeadCell className="text-end">
            <a className="cursor-pointer" onClick={handleClickProductId}>
                <div className="flex gap-1 items-center justify-end">
                  {filterProductId === "productasc" ? (
                    <FaSortAmountDownAlt />
                  ) : (
                    <FaSortAmountUp />
                  )}
                        ...
                        <a
                          ...
                          onClick={() => {
                            ...
                            try {
                              axios
                                .get(
                                  `https://quocson.fatcatweb.top/v2/review/${review.id}`
                                )
                                .then((res) => {
                  ...
                  <Dialog.Panel className="w-full md:w-1/2 flex flex-col items-center transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                    ...
                    <div className="w-full mb-6">
                      <div className="mb-3">
                        ...
                        <Textarea
                          rows={4}
                          placeholder={reviewId[0]?.comment}
                      ...
                      <div className="mb-3">
                        <div className="mb-2 block">
                          <Label htmlFor="small" value="Stars" />
                        </div>
                        <TextInput
                          type="number"
                          placeholder={reviewId[0]?.stars}
                          value={editReviewStars}
                          onChange={(e: any) =>
                            setEditReviewStars(e.target.value)
                          }
                        />
                      </div>
                      ...
                      <div className="mb-3">
                        <div className="mb-2 block">
                          <Label htmlFor="small" value="Type" />
                        </div>
                        <TextInput
                          placeholder={reviewId[0]?.type}
                          value={editReviewType}
                          type="text"
                          onChange={(e: any) =>
                            setEditReviewType(e.target.value)
                          }
                        />
                      </div>
                      ...
                      <div className="mb-3">
                        ...
                        <select
                          ...
                          onChange={(e: any) => setEditReviewProductId(e.target.value)}
                        >
                            <option
                            value={reviewId[0]?.product.id}
                            >
                            {'default: '+ reviewId[0]?.product.id + ' ' + reviewId[0]?.product.productName}
                          </option>
                          {product
                      ...
                      <div className="mb-3">
                        <div className="mb-2 block">
                          <Label htmlFor="small" value="UserId" />
                        </div>
                        <select
                          value={editReviewUserId}
                          className="border rounded-lg w-full bg-blue-100 border-blue-500 text-blue-900"
                          onChange={(e: any) =>
                            setEditReviewUserId(e.target.value)
                          }
                        >
                            <option
                            value={reviewId[0]?.user.id}
                            >
                            {'default: '+ reviewId[0]?.user.id + ' ' + reviewId[0]?.user.username}
                          </option>
                          {users
                            ? users.map((user: any) => {
                                return (
                      ...
                      <Button
                        ...
                        onClick={() => {
                          try {
                            axios
                              .patch(
                                `https://quocson.fatcatweb.top/v2/review/${reviewId[0]?.id}`,
                                {
                                  comment:
                                    editReviewComment || reviewId[0]?.comment,
                                  stars: editReviewStars || reviewId[0]?.stars,
                                  type: editReviewType || reviewId[0]?.type,
                                  user:
                                    editReviewUserId || reviewId[0]?.user.id,
                                  product:
                                    editReviewProductId ||
                                    reviewId[0]?.product.id,
                                }
                    ...
                    <div className="w-full mb-6 mt-6">
                      ...
                      <div className="mb-3">
                        ...
                        <TextInput
                          ...
                          required={true}
                          color="info"
                          onChange={(e: any) => setNewStars(e.target.value)}
                        />
                      </div>
